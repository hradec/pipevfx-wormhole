...
class gaffer(baseLib):
    ...
    def macfix(self, macfixData):
        ''' add the suffix '''
        # deal with versions
        bv = pipe.libs.version.get('boost')
        uv = pipe.libs.version.get('usd')
        pv = pipe.libs.version.get('python')
        # add the suffix to the path
        suffix = 'boost.%s-usd.%s-python%s' % (bv, uv, pv)
        macfixData['subpath'] = suffix
    ...
    def environ(self):
        ...
        # to enable cycles
        if os.path.exists(self.path('lib/libGafferUSD.so')):
            self['GAFFERCYCLES_FEATURE_PREVIEW'] = '1'

            # grab usd version from libGafferUSD
            usdVersion = cached.popen('''ldd  %s  | grep -i usd | awk '{print $(NF-1)}' | awk -F'/' '{print $(NF-2)"-usd."$(NF-3)"-gaffer."}' | sort | uniq''' % self.path('lib/libGafferUSD.so')).readlines()
            if usdVersion:
                usdVersion = usdVersion[0].strip()
